# -*- coding: utf-8 -*-
"""
Created on Tue Jul 16 15:51:44 2019
        ...
        for i in range(1, len(prices)):
            if prices[i - 1] < prices[i]:
                ...
                try:
                    ...
                    pass
                if sell:
                    price_sell.append(prices[i])
                    idx_sell.append(i)

        self.price_buy = price_buy
        self.price_sell = price_sell
        self.idx_buy = idx_buy
        self.idx_sell = idx_sell
        if price_buy == []:
            return 0
        
        elif  len(price_buy) == 1:
            return price_sell[0] - price_buy[0]
        elif len(price_buy) == 2:
            return price_sell[0] - price_buy[0] + price_sell[1] - price_buy[1]
        else:
            deal_mem = [[price_sell[0], price_buy[0]], [price_sell[1], price_buy[1]]]
            self.price_debug = [0, 0]
            self.mem_debug = [[], []]
            for i in range(2, len(price_buy)):
                deal_mem = self.dealUpdate(deal_mem, [price_sell[i], price_buy[i]])
                self.mem_debug.append([deal_mem, [price_sell[i], price_buy[i]]])
                self.price_debug.append(deal_mem[0][0] - deal_mem[0][1] + deal_mem[1][0] - deal_mem[1][1])
            return deal_mem[0][0] - deal_mem[0][1] + deal_mem[1][0] - deal_mem[1][1]
                
            
            
            
    
    def dealUpdate(self, deal_mem, deal_new):
        
        deal0 = [[deal_mem[0][0], deal_mem[0][1]],[deal_new[0],deal_mem[1][1]]]
        deal1 = [[deal_mem[1][0], deal_mem[0][1]],[deal_new[0],deal_new[1]]]
        
        s0, b0 = deal_mem[0][0], deal_mem[0][1]
        s1, b1 = deal_mem[1][0], deal_mem[1][1]
        s2, b2 = deal_new[0],deal_new[1]
        p0, p1, p2 = s0 - b0, s1 - b1, s2 - b2
        ...
        if p2 < p0 and p2 < p1:
            ...
            deal2 = [[s0, b0], [s2, b2]]
        d0 = deal0[0][0] - deal0[0][1] + deal0[1][0] - deal0[1][1]
        d1 = deal1[0][0] - deal1[0][1] + deal1[1][0] - deal1[1][1]
        d2 = deal2[0][0] - deal2[0][1] + deal2[1][0] - deal2[1][1]
        
        
        d_max = max(d0, d1, d2)
        deal_list = [deal0, deal1,deal2]
        d_list = [d0, d1, d2]
...
#prices = np.random.randint(0, 100, 100).tolist()
...
price_debug = cl.price_debug      
            
mem_debug = cl.mem_debug            
            
            
            
            
a = cl.dealUpdate([[96, 14], [99, 12]], [87, 10])          
